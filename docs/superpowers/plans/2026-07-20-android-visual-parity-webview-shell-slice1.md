...
4. **Responses shape.** Emit `my_reaction: null`, `can_moderate: false`. Extend `KotlinResponses.Comment` ADDITIVELY with `alias: Boolean`, `aliasSeed: String`, `name: String?`, `responder: String?`. Marshal each comment to `{name, avatar: null, alias, alias_seed: aliasSeed, mine: false, body, created_at}` and emit `responder` CONDITIONALLY — present (the responder's identity_pub) for a RESOLVED comment, omitted for an alias — mirroring hearth `node.py:1586-1588`'s `if resolved: comment["responder"] = identity` (app.js reads `c.responder` for the resolved-comment avatar color, so unconditional omission breaks it). For a resolved comment, `name = profileNames[identity] ?: identity[:8]` (BARE 8-char, matching hearth `node.py:1577`, NOT the "friend-"-prefixed display). A post with no responses record → `responses: null`. **[AMENDED post-Task-4 review: the original text said "OMIT responder" unconditionally — that was a misread of hearth and broke the resolved-comment shape; corrected here.]**
