...
| (c) revoke the phone's device from the desktop → phone WIPES to First-Load on next sync | **PASSED on-device — August 2026-07-23.** Revoked the phone's device from the desktop; ~30 seconds later the app wiped and returned to the First-Load screen. The full destructive path — desktop-signed revocation → phone ingest (verified own-identity self-revocation, the CRITICAL-fix gate) → `enterRevokedState` wipe (pairing.json incl identity_priv + DB + caches) → `"revoked"` event → `hasIdentity()` false → First-Load — fired end-to-end against the phone's real paired identity, backing the JVM (366/366) + real-wire (Task 8) proofs with a live device pass. |
